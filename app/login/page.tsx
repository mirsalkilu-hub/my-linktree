...
export default function LoginPage() {
  ...
  const handleCredentialResponse = async (response: any, rawNonce: string) => {
    ...
    const { error } = await supabase.auth.signInWithIdToken({
      ...
      nonce: rawNonce, // Kirimkan raw nonce ke Supabase
    });
  ...
  const initGoogleSdk = async () => {
    ...
    if (window.google?.accounts?.id) {
      ...
      setCurrentNonce(rawNonce);

      window.google.accounts.id.initialize({
        client_id: clientId,
        callback: (resp: any) => handleCredentialResponse(resp, rawNonce),
        nonce: hashedNonce, // Kirimkan hashed nonce ke Google SDK
        auto_select: false,
      });
      setGoogleReady(true);
    }
  ...
  const handleGoogleLogin = () => {
    if (!googleReady || !window.google?.accounts?.id) {
      setErrorMsg("Google SDK sedang memuat, silakan coba beberapa detik lagi.");
      return;
    }
