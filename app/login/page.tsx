...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    // 2. Dengarkan perubahan status login (otomatis terpicu saat redirect Google kembali)
  ...
  await supabase.auth.signInWithOAuth({
    provider: "google",
    options: {
      // Mengarahkan ke Supabase callback yang sudah pasti terdaftar di Google Console
      redirectTo: `https://qwnozmnrzgrdtooqepo.supabase.co/auth/v1/callback`,
    },
  });
};
    ...
    <div className="min-h-screen bg-slate-950 text-white flex flex-col justify-center items-center px-4">
      <div className="max-w-md w-full bg-slate-900 border border-slate-800 p-8 rounded-2xl shadow-2xl text-center">
        ...
        <button
          ...
          className="w-full flex items-center justify-center gap-3 bg-white text-slate-900 font-semibold py-3 px-4 rounded-xl hover:bg-slate-100 transition-all shadow-md"
        >
